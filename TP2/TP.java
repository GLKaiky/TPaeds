...
public class Personagem implements Cloneable {
    ...
    public void SelectionSort(Personagem[] personagem) {
        Scanner scanf = new Scanner(System.in);
        ...
        String a[] = ler();
        // Preencher array de personagem
        for (int i = 0; i < a.length; i++) {
            personagem[i] = new Personagem();
        }

        // Preencher respectivos personagens
        int j = 0;
        for (int i = 0; i < a.length; i++) {
            String[] partes = a[i].split(";");
            if (partes[0].equals(Arrayid.get(j))) {
                personagem[j].setId(partes[0]);
                personagem[j].setName(partes[1]);
                personagem[j].setAlternate_Names(partes[2]);
                personagem[j].setHouse(partes[3]);
                personagem[j].setAncestry(partes[4]);
                personagem[j].setSpecies(partes[5]);
                personagem[j].setPatronus(partes[6]);
                personagem[j].setHogwartsStaff(Boolean.parseBoolean(partes[7]));
                personagem[j].setHogwartsStudent(Boolean.parseBoolean(partes[8]));
                personagem[j].setActorName(partes[9]);
                personagem[j].setAlive(Boolean.parseBoolean(partes[10]));
                SimpleDateFormat dateFormatter = new SimpleDateFormat("dd-MM-yyyy");
                try {
                    Date dateOfBirth = dateFormatter.parse(partes[12]);
                    personagem[j].setDateOfBirth(dateOfBirth);
                } catch (ParseException e) {
                    e.printStackTrace();
                }
                personagem[j].setYearOfBirth(Integer.parseInt(partes[13]));
                personagem[j].setEyeColour(partes[14]);
                personagem[j].setGender(partes[15]);
                personagem[j].setHairColour(partes[16]);
                if (partes[17].equals("VERDADEIRO")) {
                    personagem[j].setWizard(true);
                } else if (partes[17].equals("FALSO")) {
                    personagem[j].setWizard(false);
                }
                j++;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        personagem[0].SelectionSort(personagem);
    }
