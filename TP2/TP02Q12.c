...
Personagem getMax(Personagem personagem[], int n) {
    Personagem maior = personagem[0];

    for (int i = 1; i < n; i++) {
        if(strcmp(maior.id, personagem[i].id) < 0){
            maior = personagem[i];
        }
    }
    return maior;
...
void radcountingSort(Personagem personagem[], int n, int exp) {
    // Array para contar o número de ocorrências de cada dígito
    int count[10] = {0};
    Personagem output[n];

    // Contar o número de ocorrências de cada dígito
    for (int i = 0; i < n; i++) {
        // Convertendo os IDs para números para usar na comparação
        int digit = personagem[i].id[0] - '0';
        count[(digit / exp) % 10]++;
    }

    // Atualizar count[i] para conter a posição correta de cada dígito no array de saída
    for (int i = 1; i < 10; i++) {
        count[i] += count[i - 1];
    }
    ...
    for (int i = n - 1; i >= 0; i--) {
        // Convertendo os IDs para números para usar na comparação
        int digit = personagem[i].id[0] - '0';
        output[count[(digit / exp) % 10] - 1] = personagem[i];
        count[(digit / exp) % 10]--;
    }

    // Copiar o array de saída para o array original
    for (int i = 0; i < n; i++) {
        personagem[i] = output[i];
    }
}


void radixsort(Personagem personagem[], int n) {
    // Encontrar o maior elemento para determinar o número de dígitos
    Personagem max = getMax(personagem, n);

    // Ordenar para cada dígito, começando pelo menos significativo
    for (int exp = 1; max.id[0] / exp > 0; exp *= 10) {
        radcountingSort(personagem, n, exp);
    }
}
