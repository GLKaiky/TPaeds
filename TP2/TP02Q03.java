...
public class Personagem implements Cloneable {
        ...
        while (true) {
            ...
            for (int i = 0; i < a.length; i++) {
                ...
                if (partes[0].equals(id)) {
                    ...
                    personagem[index].setWizard(Boolean.parseBoolean(partes[17]));
                }
            }
    ...
    public int pesquisarNome(Personagem[] personagem, int index, ArrayList<String> id) {
        int l = 0;
        while (true) {
            if (id.get(l).equals("FIM")) {
                break;
            }
            String[] a = ler();
            for (int i = 0; i < a.length; i++) {
                String[] partes = a[i].split(";");
                if (partes[0].equals(id.get(l))) {
                    personagem[index].setId(partes[0]);
                    personagem[index].setName(partes[1]);
                    personagem[index].setAlternate_Names(partes[2]);
                    personagem[index].setHouse(partes[3]);
    ...
    public int pesquisaSequencial() {
        ...
        while (true) {
            ...
            id.add(input);
            if (id.get(i).equals("FIM")) {
                while (true) {
                    input = scanf.nextLine();
                    id2.add(input);
                    if (id2.get(l).equals("FIM")) {
                        break;
                    }
                    l++;
                }
        ...
        Personagem[] personagem = new Personagem[i + 1];

        for (int k = 0; k < i + 1; k++) {
            personagem[k] = new Personagem();
        }

        int tam = pesquisarNome(personagem, 0, id);
        ...
        for (int j = 0; j < l; j++) {
            ...
            for (int k = 0; k < tam; k++) {
                comparacoes++;
                if (id2.get(j).equals(personagem[k].name)) {
    ...
    public static void main(String[] args) {
        ...
        int comparacoes = personagem.pesquisaSequencial();
        long time2 = System.currentTimeMillis();
        long total = time2 - time1;

        try {
            FileWriter escrever = new FileWriter("matrícula_sequencial.txt");
            escrever.write("802169\t");
            escrever.write(String.valueOf(total) + "ms\t");
            escrever.write(String.valueOf(comparacoes));
            escrever.close();
        } catch (IOException e) {
