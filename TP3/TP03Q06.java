...
class ListaDuplaPersonagens {
    ...
    public void QuickSort(int start, int end, Celula comeco, Celula termina) {
        int i = start;
        int k = end;
        int size = (tamanho() / 2);
        Celula top = comeco;
        Celula bot = termina;
        Celula mid = primeiro.getProx();

        for (int j = 0; j < size; j++, mid = mid.getProx())
            ;
        while (i <= jk) {

            while (top.getPersonagem().getHouse().compareTo(mid.getPersonagem().getHouse()) < 0) {
                top = top.getProx();
                i++;
            }

            while (bot.getPersonagem().getHouse().compareTo(mid.getPersonagem().getHouse()) > 0) {
                bot = bot.getAnt();
                k--;
            }
            ...
            if (i <= k) {
                Celula tmp = top;
                top.setPersonagem(bot.getPersonagem());
                bot.setPersonagem(tmp.getPersonagem());
                tmp = null;
                i++;
                k--;
            }
            ...
            if (start < k) {
                QuickSort(start, k, top, bot);
            }

            if (i < end) {
                QuickSort(i, end, top, bot);
            }
