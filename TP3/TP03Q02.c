...
void desmembraArray(NameString input[], int size)
{
    for (int i = 0; i < size; i++)
    {
        ...
        else if (strcmp(parts, "IF") == 0)
        {
            parts = strtok(NULL, " ");
            Personagem p = pesquisaporId(parts);
            inserirFim(p);
        }
        else if (strcmp(parts, "RI") == 0)
        {
            removerInicio();
        }
...
{
    start();
    Personagem personagem, support[100];
    int i = 0;
    int quant = 0;
    char id[MAX_SIZE];
