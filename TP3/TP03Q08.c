...
void inseririNoFim(Personagem personagem)
{
...
void trocarPersonagem(Celula* a, Celula* b) {
...
Celula* particionar(Celula* inicio, Celula* fim) {
    Personagem pivo = fim->personagem;
    Celula* i = inicio->ant;

    for (Celula* j = inicio; j != fim; j = j->prox) {
        if (strcmp(j->personagem.house, pivo.house) < 0 ||
            (strcmp(j->personagem.house, pivo.house) == 0 && strcmp(j->personagem.name, pivo.name) < 0)) {
            i = (i == NULL) ? inicio : i->prox;
            trocarPersonagem(i, j);
        }
    }

    i = (i == NULL) ? inicio : i->prox;
    trocarPersonagem(i, fim);

    return i;
}

void QuickSortLista(Celula* inicio, Celula* fim) {
    if (fim != NULL && inicio != fim && inicio != fim->prox) {
        Celula* pivo = particionar(inicio, fim);
        QuickSortLista(inicio, pivo->ant);
        QuickSortLista(pivo->prox, fim);
    }
}
...
int main()
{
    ...
    for(int j = 0; j < i; j++){
        inseririNoFim(personagem[j]);
    }

    QuickSortLista(primeiro, ultimo);
